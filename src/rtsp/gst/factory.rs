...
impl NeoMediaFactory {
    fn new() -> Self {
        ...
        // None: never suspend the pipeline between client disconnects. With Reset
        // (the previous setting), any camera blip that caused Frigate's ffmpeg to
        // disconnect would tear down the RTSPMedia. The data-feeding thread would
        // then pick up frames from the reconnected camera, hit check_live ("App
        // source is closed" because the bus is gone), and exit permanently. Every
        // subsequent ffmpeg reconnect got a dead pipeline. With None the pipeline
        // stays alive forever; camera drops are handled by the feed thread blocking
        // on media_rx and resuming when data flows again.
